<div id="footer-content" class="container">
  <div id="fbox1">
    <h2>Recent Updates</h2>
    <ul class="style1">
      <li class="first"><a href="http://www.andrewcollier.ca/">The collection of Andrew Collier was added</a></li>
      <li><a href="#">We've brought in some photographs from young aspiring artists</a></li>
      <li><a href="#">Art weekend in New York was a blast, mgprinting was there in full force</a></li>
    </ul>
  </div> <!-- fbox1 -->
  <div id="fbox2">
    <h2>About</h2>
    <p>This is <strong>mgprinting</strong>, a proof of concept website mocked up by Martin Gingras, using PHP, HTML, JavaScript, and CSS. To see more about Martin visit <a href="http://www.magingras.com/" rel="nofollow">magingras.com</a>.</p>
    <a href="http://www.magingras.com" class="button-style">magingras.com</a> 
  </div> <!-- fbox2 -->
  <div id="fbox3">
    <h2>Contact</h2>
    <p>Feel free to reach out to me at <a href="mailto:martin@mgingras.ca?Subject=Canvas%20Printing">martin@mgingras.ca</a>.</p>
    <ul class="style2">
      <li><a style="text-decoration:none;" href="http://github.com/mgingras/"><i class="icon-github icon-2x"></i></a></li>
      <li><a style="text-decoration:none;" href="http://www.linkedin.com/in/martinpgingras"><i class="icon-linkedin-sign icon-2x"></i></a></li>
      <li><a style="text-decoration:none;" href="https://twitter.com/martin_gingras"><i class="icon-twitter-sign icon-2x"></i></a></li>
      <li><a style="text-decoration:none;" href="mailto:martin@mgingras.ca?Subject=Your%20Website" title="Email Me!"><i class="icon-envelope-alt icon-2x"></i></li>
      </ul>
    </div> <!-- fbox3 -->
  </div> <!-- footer-content -->
</div> <!-- wrapper -->
<div id="footer">
  <p>Copyright (c) 2013 mgprinting.herokuapp.com. All rights reserved. Design by <strong>Martin Gingras from </strong><a href="http://www.magingras.com/" rel="nofollow">magingras.com</a>.</p>
</div> <!-- footer -->
</body>
</html>
